...
export async function POST(request: Request) {
  ...
  const details = fields
    .map(([key, value]) => `<tr><td style="padding:8px;border:1px solid #eadbcd"><strong>${escapeHtml(key)}</strong></td><td style="padding:8px;border:1px solid #eadbcd">${escapeHtml(value)}</td></tr>`)
    .join("");
  ...
  if (!response.ok) {
    ...
    console.error("Resend lead delivery failed", response.status, errorBody);
