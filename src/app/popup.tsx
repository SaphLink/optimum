				...
				return (
					<div className="fixed inset-0 z-[500] flex items-center justify-center p-3 sm:p-6">
						...
						<section
							...
							role="dialog"
						>
							<button aria-label="Close new client offer" className="absolute right-3 top-3 z-10 flex h-12 w-12 items-center justify-center rounded-full border-2 border-[#eadbcd] bg-white text-[#6e3e23] focus:outline-none focus:ring-4 focus:ring-[#d8b18c]" onClick={proceed} type="button">
							...
							<div className="p-5 sm:p-8 lg:p-12">
								...
								<div className="divide-y divide-[#eadbcd]">
									{offers.map((offer) => (
										<article className="flex items-center gap-3 py-4 sm:gap-5" key={offer.price}>
											<p className="shrink-0 whitespace-nowrap font-serif text-5xl font-semibold sm:text-6xl">{offer.price}</p>
											<p className="min-w-0 text-lg font-medium leading-snug sm:text-2xl">{offer.label}</p>
										</article>
									))}
								</div>
								...
								<form action={`https://formsubmit.co/${process.env.NEXT_PUBLIC_EMAIL}`} className="grid gap-3" method="POST">
